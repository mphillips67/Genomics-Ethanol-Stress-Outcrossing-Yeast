...
q <- ggplot(data3, aes(x=Media, y=DD, fill=Media)) +
  ...
  geom_boxplot()  + scale_fill_manual(breaks=c("YPD", "6% Ethanol YPD", "10% Ethanol YPD"), values =c("#4daf4a","#ff7f00","#e41a1c")) + ylim(1,2.5) + theme( axis.text=element_text(size=10),
                                                                                                                                                           axis.title=element_text(size=10,face="bold"))   + theme(legend.position="none")
...
names(doubling_time) <- c("YPD", "6% Ethanol YPD","10% Ethanol YPD")
