...
a <- ggplot(data3, aes(x=Treatment, y=DD, fill=Treatment)) +
  ...
  geom_boxplot()  + scale_fill_manual(breaks=c("Ancestor", "C Replicates", "M Replicates","H Replicates"), values =c("black", "#4daf4a","#ff7f00","#e41a1c")) + ylim(0,2) + theme( axis.text=element_text(size=10), axis.title=element_text(size=10,face="bold")) + theme(legend.title = element_text(size=15, face="bold"))  + theme(legend.position="none")
