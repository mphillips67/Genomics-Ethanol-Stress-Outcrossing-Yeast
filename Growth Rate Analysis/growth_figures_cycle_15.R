...
p <- ggplot(data2, aes(x=Time, y=OD,col=Treatment)) +  ylim(-1,.25) +
...
q <- ggplot(data3, aes(x=Treatment, y=DD, fill=Treatment)) +
  ylab('Doubling Time (hr)') +  ggtitle("B. Doubling Time in Plain YPD") +
  geom_boxplot()  + scale_fill_manual(breaks=c("Ancestor", "C Replicates", "M Replicates","H Replicates"), values =c("black", "#4daf4a","#ff7f00","#e41a1c")) + ylim(1,4) + theme( axis.text=element_text(size=10),                                                                                                                                                   axis.title=element_text(size=10,face="bold")) + theme(legend.title = element_text(size=15, face="bold"))
...
p <- ggplot(data2, aes(x=Time, y=OD,col=Treatment)) + ylim(-1,.3) +
