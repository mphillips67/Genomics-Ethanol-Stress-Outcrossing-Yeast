#script to generate manhattan plot with color coded categories
...
alpha_high <- -log10(quantile(read.table('Perms_High_1v15.txt')[,1], .005))
...
alpha_mod <- -log10(quantile(read.table('Perms_Moderate_1v15.txt')[,1], .005))
...
#control
alpha_cont <- -log10(quantile(read.table('Perms_Control_1v15.txt')[,1], .005))
