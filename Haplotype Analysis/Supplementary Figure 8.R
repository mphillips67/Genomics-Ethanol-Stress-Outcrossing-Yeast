...
#general lab selection
temp <- merge(high_sig, control_sig, by = c("chr","pos")) #shared between high and control
general <- merge(temp, moderate_sig, by = c("chr","pos")) #shared between high, control, and moderate
general_max <- general[which.min(general$pval.y), ]
...
control_focal <- control_all[control_all$chr == "C11" & abs(control_all$pos - general_max_pos) <= 30000, ]
...
moderate_focal <- moderate_all[moderate_all$chr == "C11" & abs(moderate_all$pos - general_max_pos) <= 30000, ]
...
high_focal <- high_all[high_all$chr == "C11" & abs(high_all$pos - general_max_pos) <= 30000, ]
...
x_axis_label <- textGrob("Highest General Laboratory Selection Peak - chr11:579100-639100", gp = gpar(fontsize = 15))
