...
library(GenWin)
library(dplyr)
par(mfrow=c(3,1),mar=c(2,5,0.5,2),mgp=c(3,1,0), oma=c(3,4,0,0))
#treatment + tps
...
#controls 
#control het week 1
control_week1 <- paste("alt_ETH_rep", control,"_wk01",sep="")
het_cont_week1 <- cbind(hets[,1:2],rowMeans(hets[,control_week1]))
names(het_cont_week1) <- c("chr","pos","het")
...
het_cont_week15$het <- abs(het_cont_week1$het - het_cont_week15$het)
...
temp_pos <- 173968
start <- (143715+230218+813184)/1e6
end <- (205399+230218+813184)/1e6
rect(start,0.48,end,0.5,col="gold",lty=0) #red



#legend("right",inset=-.07, "C",bty="n", cex=2)
legend("left",inset=-.12, "(A)",bty="n", cex=2)


#moderarate c1
moderate_week1 <- paste("alt_ETH_rep", moderate,"_wk01",sep="")
het_mod_week1 <- cbind(hets[,1:2],rowMeans(hets[,moderate_week1]))
names(het_mod_week1) <- c("chr","pos","het")
...
het_mod_week15$het <- abs(het_mod_week1$het - het_mod_week15$het)
...
#legend("right",inset=-.07, "C",bty="n", cex=2)
legend("left",inset=-.12, "(B)",bty="n", cex=2)
...
#high 
high_week1 <- paste("alt_ETH_rep", high,"_wk01",sep="")
het_high_week1 <- cbind(hets[,1:2],rowMeans(hets[,high_week1]))
names(het_high_week1) <- c("chr","pos","het")
...
het_high_week15$het <- abs(het_high_week1$het - het_high_week15$het )
...
legend("left",inset=-.12, "(C)",bty="n", cex=2)
...
mtext(text="Absolute difference in heterozygosity",side=2,line=0,outer=TRUE,cex=1.5)
